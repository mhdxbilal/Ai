...
PROVIDERS = {
    "openrouter": {"base_url": "https://openrouter.ai/api/v1", "api_key_env": "OPENROUTER_API_KEY", "model": "minimax/minimax-m3", "headers": {"HTTP-Referer": "https://github.com/nyldn/claude-octopus", "X-Title": "Claude Octopus OpenAI-Compatible Prototype"}},
    "pioneer": {"base_url": "https://api.pioneer.ai/v1", "api_key_env": "PIONEER_API_KEY", "model": "MiniMaxAI/MiniMax-M3", "headers": {}},
}
...
def main() -> int:
    ...
    ap.add_argument("--cwd", required=True); ap.add_argument("--max-turns", type=int, default=10); ap.add_argument("--prompt")
    ...
    messages = [
        {"role":"system","content":"You are a coding agent. Use tools when needed. For implementation tasks, edit files, call git_diff before final, and do not stop after only reading files. Final answer must be visible text."},
