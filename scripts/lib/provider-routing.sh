...
set_provider_model() {
    ...
    case "$provider" in
        codex|gemini|claude|perplexity|opencode|openrouter|openrouter-agent|pioneer-agent|openai-compatible-agent|cursor-agent) ;;
        *)
            if [[ "${4:-}" != "--force" ]]; then
                echo "ERROR: Unknown provider '$provider'. Valid: codex, gemini, claude, perplexity, opencode, openrouter, openrouter-agent, pioneer-agent, openai-compatible-agent, cursor-agent" >&2
...
reset_provider_model() {
    ...
    elif [[ "$provider" =~ ^(codex|gemini|claude|perplexity|opencode|openrouter|openrouter-agent|pioneer-agent|openai-compatible-agent|cursor-agent)$ ]]; then
